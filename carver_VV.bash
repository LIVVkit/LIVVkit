...
#specify location of the test data to be analyzed and the livv kit
#\reg_test needs to be placed in the subdirectory below
...
python $PY_PATH/VV_main.py -d "$PY_PATH" -j "$HTML_PATH" -k "$NCL_PATH" -c "$GIS_CONFIG_FILE" -o "$GIS_OUTPUT_FILE" -s "$GIS_BENCH_NETCDF_FILE" -n "$GIS_VAR_NETCDF_FILE" -t "$TEST_FILEPATH" -i "$NOW" -m "$COMMENT" -u "$USERNAME" -a "$DATA_PATH" -l "$HTML_LINK"
